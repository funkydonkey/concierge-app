...
6. РАБОТА С СУЩЕСТВУЮЩИМИ ЗАМЕТКАМИ:
   Когда пользователь хочет дополнить существующую заметку:
   - Используй list_notes() чтобы найти нужную заметку
   - ВАЖНО: Заметки могут быть в РАЗНЫХ папках (Ideas, Work, Personal, Voice Notes)
   - Если не знаешь папку - НЕ указывай параметр folder в list_notes() (поиск по всем папкам)
   - СРАЗУ вызови append_to_note() с найденным путём и новым контентом
   - Используй read_note() ТОЛЬКО если пользователь явно спрашивает "что в заметке?"

   Триггеры для ДОПОЛНЕНИЯ: "добавь к заметке", "добавь в заметку", "дополни заметку"
   Триггеры для ЧТЕНИЯ: "что в заметке про", "прочитай заметку", "покажи заметку"
...
Пример 7 - Дополнение существующей заметки:
Вход: "Добавь к заметке про экипировку для яхтинга непромокаемые шорты и перчатки"
Действие 1: list_notes(search_query="экипировка для яхтинга")
Результат: "- Ideas/2026-02-05-Экипировка для яхтинга.md"
Действие 2: append_to_note(
    note_path="Ideas/2026-02-05-Экипировка для яхтинга.md",
    content="\n- Непромокаемые шорты\n- Перчатки"
)
Комментарий: НЕ используем read_note(), сразу добавляем контент после list_notes()

Пример 8 - Вопрос о содержимом заметки:
Вход: "Что у меня в заметке про подарок для мамы?"
Действие 1: list_notes(search_query="подарок")
Комментарий: Ищем без указания папки, так как не знаем где находится заметка
Результат: "- Ideas/2026-01-20-Идея подарка для мамы.md"
Действие 2: read_note(note_path="Ideas/2026-01-20-Идея подарка для мамы.md")
Результат возвращается пользователю

ПРАВИЛА ОБРАБОТКИ:
- ВСЕГДА ВЫПОЛНЯЙ ДЕЙСТВИЯ ДО КОНЦА! Если начал list_notes - обязательно вызови append_to_note или read_note
- НЕ ОСТАНАВЛИВАЙСЯ на полпути! Не пиши "я сейчас сделаю" - ДЕЛАЙ СРАЗУ!
    ...
    async def process_transcription(self, transcription: str) -> dict:
        ...
        tools = [
            ...
            },
            {
                ...
                "function": {
                    "name": "list_notes",
                    "description": "Возвращает список заметок. Если folder не указан - ищет во всех папках (Ideas, Work, Personal, Voice Notes). Используй чтобы найти существующую заметку перед append_to_note или read_note.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "folder": {
                                "type": "string",
                                "description": "Папка для поиска (опционально): Ideas, Work, Personal, Voice Notes. Если не указано - поиск во всех папках.",
                                "enum": ["Ideas", "Work", "Personal", "Voice Notes"]
                            },
                            "search_query": {
                                "type": "string",
